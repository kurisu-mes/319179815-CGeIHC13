...
int main()
{
	...
	//luz automovil
	spotLights[2] = SpotLight(0.0f, 0.0f, 1.0f, //colores rgb
		0.5f, 0.7f, //intensidad ambiental y difusa
		0.0f, 0.0f, 0.0f, //posicion x,y,z
		0.0f, -0.2f, 1.0f, //dirección x,y,z
		1.0f, 0.0f, 0.0f, //con, lin, exp
		20.0f); //edge, tamaño de circunf.
	spotLightCount++;
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//luz auto
		glm::vec3 positionluzFaro = glm::vec3(model[3]);
		spotLights[2].SetPos(positionluzFaro + glm::vec3(0.0f, 0.0f, 7.0f));
		...
		//luz
		glm::vec3 positionluzHel = glm::vec3(model[3]);
		spotLights[3].SetPos(positionluzHel + glm::vec3(0.0f, -0.5f, 0.0f));

		//lámpara
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-8.0f, 0.8f, 3.0));
		model = glm::scale(model, glm::vec3(0.3f, 0.3f, 0.3f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
